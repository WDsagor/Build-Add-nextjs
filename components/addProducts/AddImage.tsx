...
interface AddImageProps {
  ...
  deleteHandler: (image: string, fileName?: string) => void;
  imgUrl: string[];
  imgFile: File[];
}
...
const AddImage: React.FC<AddImageProps> = ({
  ...
  const [singfile, setSingFile] = useState<File | undefined>();

  useEffect(() => {
    if (imgFile && imgFile.length > 0) {
      // Only set the first file, or handle multiple files differently
      setSingFile(imgFile[0]);
    }
  }, [imgFile]);
